...
class Teacher(nn.Module):
    ...
    def __init__(self, n_class):
        ...
        self.encoder_thermal_layer316 = Transformer(img_size=(36,64),patch_size=(2,2),in_channels=1024, hidden_size = 1024,out_channels=2048,num_attention_heads = 16)  #res34
        ########  RGB ENCODER  ########
        ...
        self.encoder_rgb_layer316 = Transformer(img_size=(36,64),patch_size=(2,2),in_channels=1024, hidden_size = 1024,out_channels=2048,num_attention_heads = 16)   #res34

        
        ########  DECODER  ########

        self.se1 = SE_fz(64,32)
        self.se2 = SE_fz(256,128)

        self.dam_p3 = DAM_Position(512)
        self.dam_p4 = DAM_Position(1024)
        self.dam_p5 = DAM_Position(2048)


        self.dam_c3 = DAM_Channel(512)
        self.dam_c4 = DAM_Channel(1024)
        self.dam_c5 = DAM_Channel(2048)
    ...
    def forward(self, input):
        ...
        rgb = self.se1(rgb)
        out1 = rgb
        if verbose: print("rgb.size() after fusion1: ", rgb.size())  
        ...
        rgb = rgb + thermal
        rgb = self.se2(rgb)
        if verbose: print("rgb.size() after fusion2: ", rgb.size())  
        ######################################################################
        ...
        out2 = rgb
        if verbose: print("rgb.size() after fusion3: ", rgb.size())  
        ######################################################################

        rgb = self.encoder_rgb_layer3(rgb)
        if verbose: print("237:rgb.size() after layer3: ", rgb.size()) 
        thermal = self.encoder_thermal_layer3(thermal)
        if verbose: print("239:thermal.size() after layer3: ", thermal.size()) 
        

        rgb = rgb + thermal
        rgb = self.dam_c4(rgb)+self.dam_p4(rgb)
        if verbose: print("248:rgb.size() after fusion4: ", rgb.size())  


        ######################################################################

        rgb = self.encoder_rgb_layer316(rgb)
        if verbose: print("254:rgb.size() after layer4: ", rgb.size()) 
        thermal = self.encoder_thermal_layer316(thermal)
...
class Reshape_fz(nn.Module):
    def __init__(self,out_channels,hidden_size,img_size, patch_size):
        super().__init__()
        #head_channels = 512
...
class Transformer(nn.Module):
    def __init__(self, img_size, patch_size,in_channels, hidden_size,out_channels,num_attention_heads):
        ...
        self.reshape_fz = Reshape_fz(out_channels,hidden_size,img_size, patch_size)
...
def unit_test():
    num_minibatch = 2
    rgb = torch.randn(num_minibatch, 3, 576, 1024).cuda(0)
    thermal = torch.randn(num_minibatch, 3, 576, 1024).cuda(0)
    images = torch.cat((rgb,thermal),dim=1)
    rtf = Teacher(2).cuda(0)
    rtf.eval()
    x = rtf(images)
